...
{{-- page title --}}
@section('title', $document->language()->title)
                ...
                <div class="display-flex media">
                    <div class="media-body">
                        <h6 class="media-heading">
                            <span class="users-view-name">{{count($document->availableLanguage) >0 ? $document->availableLanguage[0]->title : ""}} </span>
                        </h6>
                    </div>
                </div>
            </div>
            <div class="col s12 m5 quick-action-btns display-flex justify-content-end align-items-center pt-2">
                <a href="{{locale_route('document.edit',$document->id)}}" class="btn-small indigo">
                    @lang('admin.edit')
                </a>
                <a class="btn-small -settings waves-effect -light -btn right ml-3"
                   href="{{locale_route('document.destroy',$document->id)}}"
...
{{--                        </tr>--}}
                        <tr>
                            ...
                            <td>{{\Carbon\Carbon::parse($document->created_at)}}</td>
                        </tr>
                        <tr>
                            <td>@lang('admin.updated_at')</td>
                            <td>{{\Carbon\Carbon::parse($document->updated_at)}}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="row mt-2">
                    ...
                    <ul class="tabs">
                        @foreach($document->languages as $key => $language)
                            @if(isset($languages[$language->language_id]))
                                <li class="tab">
                                    <a href="#cat-{{$document->id}}-{{$language->language_id}}">
                ...
                <div class="col sm12 mt-2">
                    @foreach($document->languages as $key => $language)
                        @if(isset($languages[$language->language_id]))
                            <div id="cat-{{$document->id}}-{{$language->language_id}}"
                                 class="">
                                <table class="striped">
                                    <tbody>
                                    <tr>
